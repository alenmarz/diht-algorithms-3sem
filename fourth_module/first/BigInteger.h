...
//    const BigInteger& operator+() const;
    ...
    friend BigInteger pow(const BigInteger&, const int);
...
BigInteger &  BigInteger::operator =(const std::string & init_value) {
    BigInteger answer(init_value);
    *this = answer;
    return *this;
}

BigInteger &  BigInteger::operator =(const int init_value) {
    BigInteger answer(init_value);
    *this = answer;
    return *this;
}
...
std::string BigInteger::toString() const {
    ...
    if (!sign_ && (number_[0] != 0 || number_.size() != 1)) {
        result[0] = '-';
        offset = 1;
    }
...
const BigInteger BigInteger::operator -() const {
    ...
    if(this->number_[0] != 0 || this->number_.size() != 1) {
        other.sign_ = !(this->sign_);
    }
    return other;
}
